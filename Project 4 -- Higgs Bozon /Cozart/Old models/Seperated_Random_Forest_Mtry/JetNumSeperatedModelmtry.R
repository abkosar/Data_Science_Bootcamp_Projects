...
set.seed(0)
rf1 = randomForest(Label ~ ., data = jetnum0[,-c(1,32)], importance = TRUE,mtry=6)
rf1
...
oob.err = numeric(12)
for (mtry in 1:12) {
  fit = randomForest(Label ~ ., data = jetnum0[,-c(1,32)], mtry = mtry)
...
plot(1:12, oob.err, pch = 16, type = "b",
...
set.seed(0)
rf2 = randomForest(Label ~ ., data = jetnum1[,-c(1,32)], importance = TRUE,mtry=12)
rf2
...
oob.err2 = numeric(15)
for (mtry in 1:15) {
  fit = randomForest(Label ~ ., data = jetnum1[,-c(1,32)], mtry = mtry)
...
#Visualizing the OOB error.
plot(1:30, oob.err2, pch = 16, type = "b",
     xlab = "Variables Considered at Each Split",
     ylab = "OOB Mean Squared Error",
     main = "Random Forest OOB Error Rates\nby # of Variables")
...
set.seed(0)
rf3 = randomForest(Label ~ ., data = jetnum23[,-c(1,32)], importance = TRUE,mtry=7)
rf3
...
oob.err3 = numeric(30)
for (mtry in 1:30) {
  fit = randomForest(Label ~ ., data = jetnum23[,-c(1,32)], mtry = mtry)
...
write.csv(submission, "rfjetnum_submissionnew2.csv", row.names=FALSE)
...
write.csv(submission, "rfjetnum_submissionnew.csv", row.names=FALSE)
